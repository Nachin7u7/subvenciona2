...
function AdminTickets({ tickets, gasTypes, customers, users, onDelete }: any) {
    return (
        <div style={{ display: "flex", flexWrap: "wrap" }}>
            {tickets.map((ticket: any) => {
                ...
                const gasType = gasTypes.find((g: any) => g.id === ticket.details.gas_type_id);
                        ...
                        <CardContent sx={{ pt: 1 }}>
                            <Stack spacing={1}>
                                <Typography variant="body2">
                                    <b>Tipo de gasolina:</b> {gasType?.name}
                                </Typography>
                                <Typography variant="body2">
                                    <b>Cantidad:</b> {ticket.details.quantity_lt} lt
                                </Typography>
                                <Typography variant="body2">
                                    <b>Fecha:</b> {ticket.date.slice(0, 10)} {ticket.date.slice(11, 16)}
                                </Typography>
                            </Stack>
                        </CardContent>
                        <Divider />
